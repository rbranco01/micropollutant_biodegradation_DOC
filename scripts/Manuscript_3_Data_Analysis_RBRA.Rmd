...
data_micropol <- data_ordered %>% filter(electron_acceptor == "oxygen" & (compound == "24_D" | compound == "MCPP" | compound == "CLZ" | compound == "CLZ_DP"))
data_24_D <- data_micropol %>% filter(compound == "24_D")
data_MCPP <- data_micropol %>% filter(compound == "MCPP")
data_CLZ <- data_micropol %>% filter(compound == "CLZ")
data_CLZ_DP <- data_micropol %>% filter(compound == "CLZ_DP")
```
...
data_tfdA_total <- data_ordered_act %>% filter(compound == "tfdA_total")
```
...
### 1.2.6. Select the differen DOC fractions
...
O2 <- ggplot(data = data_resp_oxy, mapping = aes(x = time,
  ...
  scale_x_continuous(limits = c(-3, 108),
                     breaks = seq(0, 105, 20),
                     expand = c(0,0)) +
  scale_y_continuous(limits = c(-0.0375, 0.1575), 
...
breaksdoc_1 <- c("no_DOM", "humics", "dextran", "acetate", "acetate_dextran_humics")
valuesfill_2 <- c("#B2182B", "#FDDBC7", "#D1E5F0", "#2166AC", "#F7F7F7")
valuesshape_2 <- c(21, 21, 21, 21, 32)
...
functional <- ggplot(data = data_tfdA_total, mapping = aes(x = as.factor(DOM),
                                                 y = concentration,
                                                 col = community,
                                                 fill = community)) +
  ...
  geom_errorbar(aes(ymax = concentration, ymin = concentration),
                stat = "summary",
                fun.data = "mean_se",
                colour = "black",
                position = position_dodge(preserve = 'single'),
  ...
  scale_y_continuous(trans = "log10",
                    breaks = trans_breaks("log10", function(x) 10^x, n = 5),
                     labels = trans_format("log10", math_format(10^.x)),
                     expand = c(0,0)) +
  coord_cartesian(ylim = c(1e+1, 1.7e+5)) +
  facet_nested(~ community + time) +
  theme_classic() +
  theme(axis.text = element_text(size = 7, colour = "black"),
        axis.text.x	= element_text(angle = 45, hjust = 1),
...
## 3.2. Figure S2
```{r Figure S2, message=F, echo=T, eval=T, warning=T, include=T, cache=F, fig.width=6, fig.height=3.5}
tfdA_s <- ggplot(data = data_tfdA_primers, mapping = aes(x = as.factor(DOM),
                                                 y = concentration,
                                                 col = compound,
                                                 fill = compound)) +
  geom_bar(stat = "summary",
...
**Figure S2**. Concentration of tfdA gene per primer pair for no-DOC and biotic batches under aerobic conditions for each microbial community.<br>

## 3.2. Figure S3
```{r Figure S3, message=F, echo=T, eval=T, warning=T, include=T, cache=F, fig.width=3, fig.height=3}
...
**Figure S3**. Consumption of added DOC for biotic batches under nitrate reducing conditions by soil microbial community.<br>
